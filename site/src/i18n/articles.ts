 ...
 * он про интерфейс, а этот — про раздел статей, и языки в нём появляются
 * по мере перевода. Русский и английский лежат здесь, остальные — файлами
 * в articles_ui/: добавить язык значит положить рядом ещё один файл.
 */
...
export const ARTICLE_UI: Record<string, ArticleUi> = {
  ru: {
    ...
    indexTitle: 'Статьи о звуках автомобиля | Pro-Stuk',
    indexDescription:
...
// Остальные языки лежат по файлу на язык: так их правит переводчик, не рискуя
// задеть чужой. Имя файла — код языка, экспорт называется так же.
const uiFiles = import.meta.glob<Record<string, ArticleUi>>('./articles_ui/*.ts', { eager: true });
for (const [path, mod] of Object.entries(uiFiles)) {
  const lang = path.match(/([a-z]{2})\.ts$/)![1]!;
  const ui = mod[lang];
  if (ui) ARTICLE_UI[lang] = ui;
}

/** Языки, на которых раздел статей уже говорит: остальным страницы не собираем. */
export const ARTICLE_LANGS = Object.keys(ARTICLE_UI);
