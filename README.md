...
│   ├── tree_i18n/         # переводы дерева: <язык>.json, те же узлы
│   ├── schemes-parts.json # детали схем: номера и координаты (400×240)
│   ├── parts_i18n/        # подписи деталей по языкам (генерируются, см. ниже)
│   ├── gen-parts-i18n.mjs # генератор parts_i18n/ из app/lib/l10n/parts_i18n.dart
...
- `POST /api/v1/report` — multipart: `audio` (WAV PCM16 mono 16 кГц, 5–35 сек,
  ...
  Ошибки: 422 (формат/валидация, а также запись без звука автомобиля при
  пустой анкете — модель классифицирует аудио, попытка лимита возвращается),
  429 (лимит
...
Скрипт валидирует дерево, копирует `shared/tree.json`, `shared/tree_i18n/`
и `shared/schemes-parts.json` в ассеты, собирает
подписанный release APK и кладёт `stuk.apk` + `version.json` в
`site/public/app/` (каталог в `.gitignore` — это артефакты сборки).
Обновление приложения: поднять `version:` в `app/pubspec.yaml`, прогнать
скрипт заново и обновить `LATEST_APP_VERSION` в `backend/.env` — кнопка
«Проверить обновления» сравнивает именно её.

**Важно:** после публикации APK пересобрать и перезалить сайт
(`cd site && npm run build` → выгрузить `site/dist/` на хостинг), иначе
новый APK останется только на этой машине.
...
он же попадает в sitemap, canonical и robots.txt — всё генерируется
из этой константы).
...
Русский лежит в корне (`/`, `/politika/`) — его адреса уже проиндексированы;
остальные 13 языков живут в префиксе с английскими слагами: `/de/`,
`/de/how-it-works/`, `/de/privacy/`. Переведены оболочка, главная, «как это
работает» и политика; словари — `site/src/i18n/<язык>.ts`, форма задана
`types.ts`. Статьи и разборы по симптомам остаются русскими: 225 тысяч слов
переводить на тринадцать языков дороже, чем они стоят.

На нерусской главной стоит опросник — то же дерево, что в приложении, от корня;
переводы узлов берутся из `shared/tree_i18n/`, подписи деталей на схемах —
из `shared/parts_i18n/` (перегенерировать после правки `parts_i18n.dart`:
`node shared/gen-parts-i18n.mjs`).

Язык определяется как в приложении: язык браузера → ближайший язык по стране →
английский; ручной выбор в шапке сохраняется в localStorage и сильнее
автоопределения, роботов не перебрасываем. Обе проверки — часть `npm run build`:
`scripts/check-i18n.mjs` (полнота словарей и переводов дерева) и
`scripts/check-detect.mjs` (сценарии автовыбора языка на собранной странице).

```sh
cd site && npm install
npm run dev     # разработка
npm run build   # проверки словарей + статика в site/dist/ (309 страниц + sitemap)
```
...
- Глубина дерева — 2–8 вопросов вместо 3–6: решение владельца от 2026-07-31
  ради полноты покрытия (ветка запуска короче, ветки гула глубже).
- В зависимости приложения добавлен `audioplayers`: спека требует кнопку
  «Прослушать» на экране записи, но плеера в её списке зависимостей нет.
- Иконки приложения — стандартные Material Icons вместо Material Symbols
  rounded: отдельный пакет шрифта символов противоречил бы требованию
  «зависимости — минимум».
